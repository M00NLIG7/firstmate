...
result=$?
if [ "$result" = 3 ] && { [ "${FM_TEST_SCENARIO:-}" = late-ack ] || [ "${FM_TEST_SCENARIO:-}" = main-owned ] || [ "${FM_TEST_SCENARIO:-}" = main-owned-late-settlement ] || [ "${FM_TEST_SCENARIO:-}" = main-owned-cross-turn ]; }; then
  seq=${3:?}
  : > "$FM_HOME/refused-$seq"
