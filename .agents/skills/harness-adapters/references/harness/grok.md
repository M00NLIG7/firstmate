...
| Effort | `--reasoning-effort <low\|medium\|high>`, alias `--effort`; version 0.2.99 rejects `xhigh` and `max` with `use one of: high, medium, low`, so only fallback `xhigh` caps at `high`, while explicit `max` follows the unsupported-value contract in `references/common/model-and-effort.md`. |
