...
The host combines an invocation-scoped environment identity with a pre-invocation process snapshot, same-user process ownership and birth time, the fixed package working directory, and executable identity so it can retain ownership of a rapidly reparented descendant after the parent relationship disappears or the descendant replaces its environment, working directory, and executable.
