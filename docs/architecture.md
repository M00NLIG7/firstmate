...
A canonical `https://<host>/<nested-path>/-/merge_requests/<n>` URL (see [docs/gitlab-merge-watch.md](gitlab-merge-watch.md)) accepts only optional explicit squash, records and revalidates one exact source head from strict bounded `glab-axi mr view` JSON, and invokes the guarded immediate-squash `glab-axi mr merge` primitive exactly once with URL-derived host/project/IID, canonical URL, expected head, and authority.
