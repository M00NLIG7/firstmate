...
`no-mistakes` tasks run the full validation pipeline, `direct-PR` tasks open a GitHub PR or GitLab MR without that pipeline, and `local-only` tasks stay local until firstmate performs an approved fast-forward merge.
...
GitHub PR-based task merges go through `bin/fm-pr-merge.sh`, which records `pr=` and any available `pr_head=` through `bin/fm-pr-check.sh` before calling `gh-axi pr merge`.
