...
| strict invocation envelope | malformed UTF-8, a byte-order mark, unescaped controls, malformed or multiple JSON documents, duplicate or unknown fields, oversized stdout, oversized stderr, wrong request ids, crashes, nonzero exits, a successful parent that leaves a detached background descendant, a parent that exits before its detached descendant can be observed through PPID ancestry, and authority-shaped result fields are rejected; leaked descendants are reaped and package diagnostic text is not copied into the bounded host-produced error evidence |
