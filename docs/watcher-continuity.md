...
Pi same-process session replacement follows the generation-owner contract in `.pi/extensions/fm-primary-pi-watch.ts`: an owning `session_start` arms the replacement generation without waiting for a model turn, and its existing pending handoff carries overlapping, deferred and unacknowledged delivery through replacement or interruption.
The extension header owns main delivery, consumption and source-acknowledgement semantics; transport submission never substitutes for handling, and successor creation never waits for the model.
...
If a branch offer loses the claim race to main, it marks that ownership result explicitly and rejects its settlement so the watcher retains delivery; a real branch failure is not classified from similar error text.
...
Pi can combine proved working-only signal notifications before main processes them, without clearing human input or changing the selected model.
Only fully proved working-only signals qualify; decision, blocker and check notifications remain outside that grouping, and missing acknowledgement evidence cannot justify silence.
The pending-delivery contract is owned by the Pi extension header, and `bin/fm-wake-drain.sh` owns its optional bounded acknowledgement-evidence format without changing actor claims or acknowledgement authority.
...
It also verifies grouped records surviving native consumption until source acknowledgement, owner loss, reload, stale callbacks and real branch failure.
The token-free portion of `tests/fm-pi-primary-live-e2e.test.sh` exercises actual Pi queues and provider invocations, delayed acknowledgement, mixed human/urgent input, interrupted handling and cold recovery; [the current verification record](verification/runtime-backends.md#2026-09-12-pi-pending-main-delivery) owns its results and limits.
