...
When a live successor does not establish within its first shared readiness bound, Pi surfaces each queued failure once without starting an overlapping replacement and continues later closures in order; delayed establishment cannot replay a released close.
...
It also holds a Pi successor unready past the shared bound, verifies that neither notification nor an overlapping child escapes early, and proves that the queued failure and later actionable close deliver once in order while delayed readiness causes no replay.
