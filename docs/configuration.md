...
`glab-axi` is intentionally task-scoped rather than part of the universal toolchain because its validated standalone distribution currently has no install target.
Firstmate neither fabricates an installer nor exposes the distribution's separately built `glab` compatibility executable as a global CLI.
Universal bootstrap support requires a separately shipped installation surface plus an approved install recipe and version-floor policy; until that exists, homes without GitLab work remain usable and a GitLab task that needs `glab-axi` stops and reports the missing tool.
